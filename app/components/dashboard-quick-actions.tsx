...
import { MessageSquare, AlertTriangle, Bell, X } from "lucide-react"
...
export function DashboardQuickActions() {
  ...
  return (
    <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50">
      ...
      <div
        ref={toolbarRef}
        className="bg-[#0A0F1C]/90 backdrop-blur-md border border-blue-900/30 rounded-full shadow-lg shadow-blue-900/20 px-3 py-2 flex items-center gap-3 transition-all"
      >
        {/* AI Section */}
        <div className="flex items-center gap-4">
          <Button
            variant="ghost"
            size="sm"
            className="rounded-full flex items-center gap-2 relative text-blue-100 hover:bg-blue-900/30 hover:text-blue-50 active:bg-blue-900/40"
            onClick={() => {
          ...
          >
            <div
              className={cn(
                ...
                aiStatus === "ok" ? "bg-blue-500" : "bg-blue-500 animate-pulse",
              )}
            ></div>
            <span className="font-medium text-[14px]">{getAIStatusText()}</span>
            {unreadCount > 0 && (
              <span className="bg-blue-500 text-white text-xs rounded-full px-1.5 py-0.5 font-medium min-w-[20px] text-center">
                {unreadCount}
              </span>
            )}
          </Button>
          ...
          <Button
            variant="ghost"
            size="sm"
            className="rounded-full text-blue-100 hover:bg-blue-900/30 hover:text-blue-50 active:bg-blue-900/40 flex items-center gap-2"
            onClick={() => {
